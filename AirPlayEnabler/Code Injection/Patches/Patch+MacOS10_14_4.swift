//
//  Patch+MacOS10_14_4.swift
//  AirPlayEnabler
//
//  Created by Darren Mo on 2019-04-12.
//  Copyright © 2019 Darren Mo. All rights reserved.
//

extension Patch {
   static func makePatchesForMacOS10_14_4() -> [Patch] {
      return [
         makeInjectedCodePatch(),
         makeFunctionCallPatch()
      ]
   }

   private static func makeInjectedCodePatch() -> Patch {
      // c.f. "Injected Code/macOS 10.14.4/".
      return Patch(addressInExecutableFile: 0x100007af8,
                   requirements: [Requirement(addressInExecutableFile: 0x100002908,  // _NSConcreteGlobalBlock check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0x48, 0x8D, 0x35, 0xC9, 0x4A, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x100001e36,  // __CFConstantStringClassReference check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0x4C, 0x8D, 0x2D, 0x13, 0x59, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x1000016b9,  // ___stack_chk_guard check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0x48, 0x8B, 0x05, 0x78, 0x59, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x100001998,  // __stack_chk_fail check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0x81, 0x35, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x100001ef7,  // os_log_create check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0xAC, 0x30, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x100001715,  // os_log_type_enabled check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0x94, 0x38, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x100001b56,  // _os_log_impl check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0xD5, 0x33, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x100001763,  // _os_log_error_impl check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0xBC, 0x37, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x10000296f,  // dispatch_once check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0xE0, 0x25, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x100002105,  // SecRequirementCreateWithString check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0xE4, 0x2D, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x100002172,  // SecStaticCodeCheckValidityWithErrors check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0x83, 0x2D, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x10000192f,  // CFRelease check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0x48, 0x35, 0x00, 0x00]))),
                                  Requirement(addressInExecutableFile: 0x10000328f,  // sub_10000436b check
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0xE8, 0xD7, 0x10, 0x00, 0x00])))],
                   target: MemoryData(littleEndianData: Data(repeating: 0, count: 0x508)),
                   replacement: InjectedCode(memoryData: MemoryData(littleEndianData: Data([])),
                                             absoluteAddressRanges: [0x4a7..<0x4af,  // __block_descriptor_tmp
                                                                     0x4c7..<0x4cf,  // __block_literal_global
                                                                     0x4cf..<0x4d7,
                                                                     0x4e8..<0x4f0],  // cfstring_anchor_apple_generic_and__certificate_leaf_field_1_2_840_113635_100_6_1_9__or_certificate_1
                                             externalSymbolInfos: [InjectedCode.ExternalSymbolInfo(absoluteAddressRange: 0x4b7..<0x4bf,  // External Symbol: _NSConcreteGlobalBlock
                                                                                                   replaceWithPointerValueAt: 0x1000073d8),
                                                                   InjectedCode.ExternalSymbolInfo(absoluteAddressRange: 0x4d8..<0x4e0,  // External Symbol: __CFConstantStringClassReference
                                                                                                   replaceWithPointerValueAt: 0x100007750)],
                                             writableRanges: [0x497..<0x4a7,  // __block_descriptor_tmp
                                                              0x4af..<0x4b7,
                                                              0x4bf..<0x4c7,  // __block_literal_global
                                                              0x4f8..<0x500,  // _RunInjectedCode.log
                                                              0x500..<0x508]),  // _RunInjectedCode.onceToken
                   originalMemoryProtection: VM_PROT_READ | VM_PROT_WRITE)
   }

   private static func makeFunctionCallPatch() -> Patch {
      // Replaces `call sub_10000436b` with `call _RunInjectedCode`.
      // The requirements ensure that the arguments to the call are what we expect.
      return Patch(addressInExecutableFile: 0x10000328f,
                   requirements: [Requirement(addressInExecutableFile: 0x100003277,
                                              requiredMemoryData: MemoryData(littleEndianData: Data([0x48, 0x8B, 0x4D, 0x30, 0x48, 0x8D, 0x95, 0x7C, 0xFF, 0xFF, 0xFF, 0xC7, 0x02, 0x00, 0x00, 0x00, 0x00, 0x48, 0x8B, 0x7D, 0x98, 0x4C, 0x89, 0xF6])))],
                   target: MemoryData(littleEndianData: Data([0xE8, 0xD7, 0x10, 0x00, 0x00])),
                   replacement: InjectedCode(memoryData: MemoryData(littleEndianData: Data([0xE8, 0x64, 0x48, 0x00, 0x00]))),
                   originalMemoryProtection: VM_PROT_READ | VM_PROT_EXECUTE)
   }
}
